...
async function storeIn1Password(name, value, update = false) {
  ...
  try {
    ...
    if (update) {
      ...
      execSync(`tmux -S ${sock} send-keys -t ${session}:0.0 -- ${JSON.stringify(`op item create --category "API Credential" --title "${title}" --vault "${OP_VAULT}" "credential[password]=${value}" "username[text]=lancejames@unit221b.com" 2>&1`)} Enter`, { stdio: 'pipe' });
    }
